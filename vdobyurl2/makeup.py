...
def distribute_mulist(opath, time=2):
    ...
    with open(mupath, "r") as f:
        for line in f:
            mulist.append(line)
    wholelen = int(len(mulist)/2)
    lack = time - wholelen%time
    if lack != time:
        for _ in range(lack):
            mulist.append('\n')
    wholelen = int(len(mulist)/2)
    blocklen = int(wholelen/time)

    for i in range(time):
        opaths.append(f'{opath}/{i}/')
        os.makedirs(opaths[i], exist_ok=True)
        mupathi = f'{opaths[i]}/mulist.txt'
        fi = open(mupathi, "w")
        ...
        print(f'from {jstart}th to {jend}th murls are written to the {i}th file\n')
    return opaths
...
if __name__ == "__main__":
    ...
    # time.sleep(24*60*60)
    makeuplist = [
        # './vdo4mnt/recent/260629/',
        './vdo4mnt/recent/2026-07-09/0/',
        './vdo4mnt/recent/2026-07-09/1/',
        ]
    # newmakeuplist = []
    # for makeuppath in makeuplist:
    #     list = distribute_mulist(makeuppath)
    #     newmakeuplist.extend(list)
    makeup(makeuplist)
