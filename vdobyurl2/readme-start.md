...
### version 2
该网页构造形如https://mc.jl.com:15610/index.m3u8?sign=20f5a74402032469eb1102ce641155ff&t=1778608109的请求链接，为拿到sign与t，只得进一步改进无头浏览器，模拟完整执行流程获取资源

最终本方案改进my16brow浏览器获取重定向方案，通过及时重试获取新url；实现请求层find_m3u8，动态获取网站分配的m3u8链接；另外附加实现多线程下载，充分利用网络带宽，从而高效获取资源。

弊端：重试获取重定向url不稳定，且该版本的两个子版本无法自动处理<获取到新url被程序误认旧url\不能及时获取新url>的矛盾。

### version 3
将在v3中实现：通过守护进程，及时更新重定向url，而不再令进程自行获知重定向。daily下载的自启动。
